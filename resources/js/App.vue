<template>
	<div class="flex flex-col app-template" :class="localeClass">
		<!-- HEADER -->
		<header class="flex flex-row items-center justify-between app-header">
			<!-- {{-- SVG --}} -->
		...
		<div
			v-if="isSuccess && isModal"
			class="flex flex-col justify-center flex-grow success-view"
		>
		...
		<div
			v-if="!isSuccess && isModal"
			class="relative flex flex-col justify-center flex-grow mx-auto modal-view"
		>
			<div @click="toggleModal" class="absolute cursor-pointer top-1 right-1">
				<svg
					xmlns="http://www.w3.org/2000/svg"
					width="16"
					height="16"
